function [frequency,Phase_shift] = PS_TLC (D1,D2,Gc,Gw,time,dt)
...
nprops(1,:) = [C;R;Gm;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;D1]';
nprops(2,:) = [C;R;Gm;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;D1]';
nprops(3,:) = [C;R;0;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;0]';
nprops(4,:) = [C;R;0;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;0]';

nprops(5,:) = [C;R;Gm;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;D2]';
nprops(6,:) = [C;R;Gm;Eca;Vr;Tm;Sm;VmidM;Th;Sh;VmidH;0;D2]';
...
V_RG_E = nstate(1,:,1); V_RG_F = nstate(1,:,2);
V_PF_E = nstate(1,:,5); V_PF_F = nstate(1,:,6);

[F_RG,SS_RG] = ff(V_RG_E,V_RG_F,dt);
[F_PF,SS_PF] = ff(V_PF_E,V_PF_F,dt);

%% Phase Delay
Relative_frequency_difference = (F_RG - F_PF);

if Relative_frequency_difference == 0
%     keyboard
    pks_RG = length(findpeaks(V_RG_E(time/(5*dt):end)));
    pks_PF = length(findpeaks(V_PF_E(time/(5*dt):end)));
    
    if abs(pks_RG-pks_PF) < 2
        if length(SS_RG)>length(SS_PF)
            d = length(SS_RG)-length(SS_PF);
            SS_RG = SS_RG(d+1:end);
        elseif length(SS_RG)<length(SS_PF)
            d = length(SS_PF)-length(SS_RG);
            SS_PF = SS_PF(d+1:end);
        end
        
        SSD = SS_RG-SS_PF;
        MD = mean(SSD);
        Phase_shift = 100*MD/(1000/F_RG); % percent
    else
        Phase_shift = NaN;
    end
