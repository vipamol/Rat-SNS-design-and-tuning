function [Drive,Descend] = R_TLC (D1,D2,Gc,Gw,time,dt)
...
[nstate,syn_state] = Current_CPG(nstate,nprops,sstate,sprops,conn_map,ext_stim,time,dt);
...
if F == 0
    Drive = [D1;D2];
    Descend = [syn_state(3,:,7);syn_state(3,:,8)];
else
    Drive = NaN;
    Descend = [syn_state(3,:,7);syn_state(3,:,8)];
end
