function [ neur_state, neur_derivatives,V_null ] = simulate( neur_state, neur_props, syn_state, syn_props, syn_map, ext_cond, time, dt )
...
%             keyboard
            [syn_state(:,i,j),syn_null(:,i,j)] = NSCS_func(syn_state(:,i-1,j),syn_props(j,:));
        end
        
        for j=1:num_neurons
                neur_state(5,i-1,j) = ext_cond(i-1,j) + sum(syn_state(3,i-1,post_syn_ind{j}));  
                [neur_state(:,i,j),neur_derivatives(:,i,j)] = NSN_func(neur_state(:,i-1,j),neur_props(j,:),dt);
                
                V_null{i,j} = syn_null(:,i,post_syn_ind{j});
        end
%         keyboard
    end
end
