function PF_G = find_G_PF(D,Gw,Gc,G_syn,ext_stim,time,dt)
...
nstate = simulate(nstate,nprops,sstate,sprops,conn_map,ext_stim(:,1:4),time,dt); 
[frequency,~] = ff(nstate(1,:,1),nstate(1,:,2),dt);

% keyboard

%% Find the right conductance for PF to achieve same frequency

% f = @(x)f_G_TLC (D,Gw,Gc,G_syn,x,ext_stim,time/8,dt)-frequency;
% rg = bisect(f,1.4,1.63,1e-12,1e-12,1000);  
% r = round (rg,3);
% % keyboard
f = @(x)f_G_TLC (D,Gw,Gc,G_syn,x,ext_stim,time,dt)-frequency;
PF_G = bisect(f,1.612,1.613,1e-12,1e-12,1000);  
