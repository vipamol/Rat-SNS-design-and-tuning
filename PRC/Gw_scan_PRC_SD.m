...
if to_simulate
    ...
    if ~design_low_eq_pt
        ...
        [v_start_low,v_start_high,v_start_interneuron_inh,v_start_interneuron_exc] = I_find_inhibited_V_ss([],[],nprops,sprops);
    else
        
        v_start_low_des = V_th_low + delta;
        
        f = @(x)I_find_inhibited_V_ss(x,13,nprops,sprops) - v_start_low_des;
        I_stim = bisect(f,0,10,1e-12,1e-12,1000)
        
        [v_start_low,v_start_high,v_start_interneuron_inh,v_start_interneuron_exc,nprops,sprops] =I_find_inhibited_V_ss(I_stim,13,nprops,sprops);
    end
    ...
    sgtitle(['\delta = ',num2str(delta),'     G_w = ',num2str(Gw)])
...
%         disp(['i = ',num2str(i),', there are ',num2str(num_eq_pts(i)),' equilibrium points.']);
        ...
        F(i) = fs(nstate(1,:,1),nstate(1,:,2),dt);
    end
...
phase_shift = zeros(length(Drive),num_phase);
