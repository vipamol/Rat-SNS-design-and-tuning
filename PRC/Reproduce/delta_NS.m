...
if to_simulate
    ...
    sprops(3,:) = [1.041;-100;V_th_low;-20];
    sprops(4,:) = [1.041 ;-100;V_th_low;-20];
...
%         disp(['i = ',num2str(i),', there are ',num2str(num_eq_pts(i)),' equilibrium points.']);
    ...
    set(gcf,'Position',[500 200 300 150])
...
for i = 1:length(G)
    ...
    ext_stim = zeros(t_max/dt,4);
    nstate = HC_NS(Gw,G_syn,ext_stim,t_max,dt);
    ...
    parfor j = 1:num_phase
        ...
        nstate = HC_NS(Gw,G_syn,sim_ext_stim,sim_time,dt);
...
legend('\delta = 0.1','\delta = 1','\delta = 3')