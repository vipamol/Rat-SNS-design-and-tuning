...
for i = 1:length(G)
    ...
    ext_stim = zeros(t_max/dt,4);
    nstate = HC_AJH(G_syn,ext_stim,t_max,dt);
    [frequency,step_start] = ff(nstate(1,:,1),nstate(1,:,2),dt);
    
    % set up tracking time.
    acr = step_start(1);
    sim_time = acr + round(phase_to_track*1000/frequency);
    stim_time = round(1000/(frequency*num_phase));
    
    % PRC of CPG
    for j = 1:num_phase
        sim_ext_stim = zeros(sim_time/dt,4);
        sim_ext_stim(acr+stim_time*(j-1):acr+stim_time*j,3) = ramp_mag;
        nstate = HC_AJH(G_syn,sim_ext_stim,sim_time,dt);
        [~,step_start_new] = ff(nstate(1,:,1),nstate(1,:,2),dt);
        if j <11
            phase_shift(i,j) = (step_start_new(end-1)-step_start(2))*frequency/1000;
        else
            phase_shift(i,j) = (step_start_new(end)-step_start(3))*frequency/1000;
        end
        %     figure(1)
        %     hold on
...
legend('\delta = 0.01','\delta = 0.1','\delta = 0.3','\delta = 0.5')