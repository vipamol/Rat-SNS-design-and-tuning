...
if to_simulate
    ...
    if ~design_low_eq_pt
        ...
        [v_start_low,v_start_high,v_start_interneuron_inh,v_start_interneuron_exc] = Gw_find_inhibited_V_ss([],[],nprops,sprops);
    else
...
if to_iterate   
    ...
    for i = 1:length(Deltas)
        ...
        f = @(x)Gw_find_inhibited_V_ss(x,14,nprops,sprops) - v_start_low_des;
        Gw = bisect(f,0,5,1e-12,1e-12,1000);
...
% delta = [0.01;0.1;0.3]; 
G_syn = 2;
Gweak = [0.556780360355788,0.629572679138164,0.796516782398157];
% Gweak = 0.556780360355788;
D = 0;
...
%         figure (1)
%         hold on
%         plot(nstate(1,acr:acr+round(2000/frequency),2));
